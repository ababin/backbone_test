...
CuteGrid = function (map){
	...
	CuteGrid.prototype.prepareTableCaption = function(){
		if(this.caption != null && this.caption != undefined){
			...
			var html = '';
			html += '<tr class="caption"><th colspan="' + columnsCount + '">';
			html += this.caption;
			html += '<div class="panel">';
			html += '<a href="javascript:' + this.component + '.createNew()" class="glyphicon glyphicon-plus"></a>';
			html += '<a href="javascript:' + this.component + '.reshow()" class="glyphicon glyphicon-refresh"></a>';
			html += '</div>';
			html += '</th></tr>';
			return html;  
		}
	...
	CuteGrid.prototype.createNew = function(){
		...
		var val = Admin.types.Factory.createType('12345', 'Task','title');
		alert(val);
		//dialogManager.showDialogFor(this.component , -1 , 'C');
	};
